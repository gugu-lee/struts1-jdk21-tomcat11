...
public class ActionConfig extends BaseConfig {
     ...
     * any, keyed by the <code>type</code> property.
     * </p>
     */
    ...
    /**
     * <p>
     * Fully qualified Java class name of the <code>Action</code> class to
    ...
    /**
     * <p>
     * Should the <code>validate()</code> method of the form bean
    ...
    /**
     * <p>
     * The internal name of this action mapping. If an action has a name, it may be
     * used
     * as a shortcut in a URI. For example, an action with an identification of
     * "editPerson"
     * may be internally forwarded as "editPerson?id=1" which will then resolve to
     * the
    ...
    protected void inheritExceptionHandlers(ActionConfig baseConfig)
        ...
        for (int i = 0; i < baseHandlers.length; i++) {
            ...
            if (copy == null) {
                ...
                copy = (ExceptionConfig) RequestUtils.applicationInstance(baseHandler.getClass()
                        .getName());
    ...
    protected void inheritForwards(ActionConfig baseConfig)
        ...
        for (int i = 0; i < baseForwards.length; i++) {
            ...
            if (copy == null) {
                // We don't have this, so let's copy it
                copy = (ForwardConfig) RequestUtils.applicationInstance(baseForward.getClass()
                        .getName());
    ...
    /**
     * <p>
     * Add a new <code>ExceptionConfig</code> instance to the set
     ...
     * handler configuration can be found, return <code>null</code>.
     * </p>
     *
     * <p>
     * Introduced in <code>ActionMapping</code> in Struts 1.1, but pushed
     * up to <code>ActionConfig</code> in Struts 1.2.0.
     * </p>
     *
